...
class GetAddressViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        zAddressLabel.text = (try? derivationTool.deriveShieldedAddress(seed: DemoAppConfig.seed, accountIndex: _accountIndex)) ?? "No Addresses found"
    ...
    */
    ...
    @IBAction func addressTapped(_ gesture: UIGestureRecognizer) {
        loggerProxy.event("copied to clipboard")
        UIPasteboard.general.string = try? DerivationTool(networkType: ZCASH_NETWORK.networkType).deriveShieldedAddress(seed: DemoAppConfig.seed, accountIndex: _accountIndex)
